  ...
  )
{
  ...
  ApicBaseMsr.Uint64 = ReadMsrReg (MSR_IA32_APIC_BASE);
  ...
  )
{
  ...
  WriteMsrReg (MSR_IA32_APIC_BASE, ApicBaseMsr.Uint64);
}
  ...
  )
{
  ...
  if (GetApicMode () == LOCAL_APIC_MODE_XAPIC) {
    Ret = MmioRead32 (GetLocalApicBaseAddress() + MmioOffset);
  } else {
    //
    ...
    Ret = (UINT32)ReadMsrReg (MsrIndex);
  }
  return Ret;
}
  ...
  )
{
  ...
  if (GetApicMode () == LOCAL_APIC_MODE_XAPIC) {
    ...
    //
    MemoryFence ();
    WriteMsrReg (MsrIndex, (UINT64)Value);
  }
}
